...
DOCLING_MAX_SYNC_WAIT="${LOCAL_FIREPDF_DOCLING_MAX_SYNC_WAIT:-900}"
...
Environment:
  ...
  LOCAL_FIREPDF_DOCLING_MAX_SYNC_WAIT=$DOCLING_MAX_SYNC_WAIT
...
start_docling() {
  if docker ps --format '{{.Names}}' | grep -qx "$DOCLING_CONTAINER"; then
    ...
    docker run -d \
      ...
      -e "DOCLING_SERVE_MAX_SYNC_WAIT=${DOCLING_MAX_SYNC_WAIT}" \
...
LOCAL_FIREPDF_MAX_CONCURRENT_OCR=${LOCAL_FIREPDF_MAX_CONCURRENT_OCR:-2}
LOCAL_FIREPDF_FAIL_LOW_QUALITY=${LOCAL_FIREPDF_FAIL_LOW_QUALITY:-true}
...
LOCAL_FIREPDF_DOCLING_MAX_SYNC_WAIT=${LOCAL_FIREPDF_DOCLING_MAX_SYNC_WAIT:-900}
