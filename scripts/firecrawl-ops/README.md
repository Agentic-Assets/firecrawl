...
| `cre_listing_om_facts` | OM/PDF-parsed underwriting facts (013; OM-parse tier gated) |
...
run-health. Migrations `009` through `014` are applied to prod; OM-parse,
live status activation, consumer board-gate deploy, media backfill, and weekly
mark-missing escalation (`CRE_WEEKLY_MARK_MISSING=1`) stay gated for explicit
go-ahead.
