...
|--------|--------------------------------------|-----------|
| **ACTIVE** | `cre_collector/collect.ts` | Production collect CLI (20 source keys) |
| **ACTIVE** | `cre_collector/sources/*.ts` | Per-broker adapters that run in production |
| **ACTIVE** | `cre_collector/cre_ingest.py` | Supabase `credeals` upsert |
| **ACTIVE** | `cre_collector/cre_daily_update.sh` | Scheduled full refresh (launchd) |
...
**Production path (the only daily pipeline):** broker adapters live in
