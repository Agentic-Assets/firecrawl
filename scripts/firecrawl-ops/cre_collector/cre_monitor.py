...
def load_artifact_groups(paths):
    ...
    return groups, run_started_at, dict(per_source_flat), skipped_no_url
...
def main():
    ...
    groups, run_started_at, per_source_flat, skipped_no_url = load_artifact_groups(args.inputs)
    if not groups:
    ...
    if apply_mode:
        ...
        for sk in run_source_keys:
            if sk in baseline_source_keys:
                coverage_ok_by_source[sk] = None
                continue
            prior_live = prior_live_count_by_source.get(sk, 0)
            if args.force_disappear or prior_live == 0:
                coverage_ok_by_source[sk] = True
            else:
                ok = enum_count_by_source.get(sk, 0) >= DISAPPEAR_COVERAGE_FRACTION * prior_live
                coverage_ok_by_source[sk] = ok
                if not ok and not args.quiet:
                    print(
                        f"WARNING: coverage gate failed for source '{sk}': enumerated "
                        f"{enum_count_by_source.get(sk, 0)} < {DISAPPEAR_COVERAGE_FRACTION} * "
