...
const COLLIERS_MAIN_HOST = "https://www.colliers.com";
const COLLIERS_MAIN_SITEMAP_INDEX = `${COLLIERS_MAIN_HOST}/sitemap`;
const COLLIERS_MAIN_SOURCE_URL = `${COLLIERS_MAIN_HOST}/en/properties`;
const COLLIERS_MAIN_DETAIL_CONCURRENCY = boundedInt(
...
const COLLIERS_MAIN_DETAIL_WAIT_MS = boundedInt(process.env.COLLIERS_MAIN_DETAIL_WAIT_MS, 4000, 0, 30000);
const COLLIERS_MAIN_SCRAPE_OPTS: ScrapeOpts = {
  proxy: "stealth",
  timeout: 120000,
  ...(COLLIERS_MAIN_DETAIL_WAIT_MS ? { waitFor: COLLIERS_MAIN_DETAIL_WAIT_MS } : {}),
};

type ColliersMainEntry = { url: string; lastmod: string | null; id: string };

let colliersMainSitemapCache: ColliersMainEntry[] | null = null;
let colliersMainEnrichedMemo: any[] | null = null;

function colliersMainIsChallenge(doc: ScrapedDoc): boolean {
...
async function scrapeColliersMainDetailDoc(url: string): Promise<ScrapedDoc> {
...
function colliersMainAbs(href: string | null | undefined): string | null {
  const h = clean(href);
...
function colliersMainIdFromUrl(url: string): string | null {
...
function extractSitemapUrlEntries(xml: string): Array<{ loc: string; lastmod: string | null }> {
...
async function fetchColliersMainEntries(): Promise<ColliersMainEntry[]> {
...
function colliersMainTransaction(
  ldName: string | null,
  markdown: string,
  url: string
...
function parseColliersMainAddress(addr: string | null): {
  street: string | null;
  city: string | null;
  state: string | null;
  postalCode: string | null;
...
function colliersMainJsonLd(rawHtml: string): any | null {
...
function parseColliersMainDetail(entry: ColliersMainEntry, doc: ScrapedDoc): any {
...
function appendColliersMainCache(path: string, listing: any): void {
  if (listing?.detailError) return;
  mkdirSync(dirname(path), { recursive: true });
  appendFileSync(path, `${JSON.stringify(listing)}\n`);
}
...
async function colliersMainEnrichAll(max: number): Promise<any[]> {
