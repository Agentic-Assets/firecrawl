...
| `marcus-millichap` | Public map ActivityId feed, mappropertydetail tiles, plus public detail HTML | 105 in no-ingest probe, 3,126 reported public sale total | n/a | Sale-only until public lease inventory is proven; full sale run and ingest validation pending; detail enriches contacts/images and keeps gated deal-room URLs in raw data only |
