...
| 8 | Make the existing no-network suite a local release gate | H | L | Well-built suite should run before branch push and merge readiness |
...
7. **Make the pytest/TS suites a local release gate** (H/L). The no-network
   suite should run before branch push and merge readiness, with its exact
...
22. **Local disposable-Postgres integration test for `cre_ingest.py`** (M/M). The SQL generator needs a local Docker Postgres contract run loaded from `sql/000_run_all.sql`, with one real non-dry-run ingest, to verify COALESCE-keep, mark-missing gating, and circuit-breaker semantics against actual constraints. Do not add GitHub Actions CI.
