...
## 2. Fresh evidence that changes the prior plan

### 2.1 Listing runtime

- The local Firecrawl healthcheck passed on 2026-07-11: containers were up, the
  API root responded, and a scrape smoke test succeeded.
- Monitor is current and green.
- The newest enrich run claimed 200 rows and enriched 200 of 200 in about 119
  seconds. It then failed during Postgres ingest and released all 200 claims.
- The failure is deterministic: production has the verified five-column
  `cre_listing_om_facts` key, but `cre_ingest.py` still emits:
  `ON CONFLICT (listing_id, fact_group, fact_key, source_doc_url)`.
- Daily remains loaded even though the dispatcher calls it retired and
  rollback-only.
- Weekly's current launchd exit is 0, but its last durable run marker is stale
  and failed. Treat the marker as the authoritative evidence until a fresh
  weekly run completes.

This makes the collector writer mismatch the first blocker. Firecrawl itself
