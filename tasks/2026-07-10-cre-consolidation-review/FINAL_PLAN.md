...
2. **Pick GetCREdata's scheduling lane and finish it.** Its GH Actions cron was demoted 2026-07-08 and the aa-hub job is a disabled stub pointing at a wrong host path, so today it has **no automated refresh at all** and the chatbot table is going stale. Either restore the GH Actions cron (worked before) or finish the aa-hub lane; the half-migrated state is worse than either.
...
4. GetCREdata scheduling lane: restore GH Actions cron, or finish the aa-hub job.
