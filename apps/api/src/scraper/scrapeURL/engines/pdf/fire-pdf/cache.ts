...
// instead of serving cached OCR, and local Docling profile/env settings can
// change under identical PDF bytes. Calls with `maxPages` also bypass because
// the cached entry may have been written with a different cap.
...
export function cacheKeyShape(
  ...
  const baseVariant: string | undefined = undefined;
  ...
  const lookupVariants: (string | undefined)[] = includePageMarkdown
    ? [PAGE_MARKDOWN_VARIANT]
    : [undefined, PAGE_MARKDOWN_VARIANT];
