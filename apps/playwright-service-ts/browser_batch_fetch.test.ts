...
test("rejects credentials and cross-origin requests", () => {
  assert.throws(
    () =>
      parseBrowserBatchFetchInput({
        ...validInput(),
        bootstrapUrl: "https://user:pass@example.com/",
      }),
  ...
  assert.throws(
    () => parseBrowserBatchFetchInput(crossOrigin),
    /bootstrap origin/,
  );
});
...
test("rejects methods, headers, and bodies outside the narrow contract", () => {
  ...
  assert.throws(
    () => parseBrowserBatchFetchInput(authHeader),
  ...
  assert.throws(
    () => parseBrowserBatchFetchInput(oversizedBody),
...
test("rejects oversized batches and aggregate bodies", () => {
  ...
  assert.throws(
    () => parseBrowserBatchFetchInput(aggregate),
...
test("rejects URL fragments and unsupported content types", () => {
  ...
  assert.throws(
    () => parseBrowserBatchFetchInput(fragmented),
  ...
  assert.throws(
    () => parseBrowserBatchFetchInput(unsupported),
