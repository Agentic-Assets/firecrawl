...
export class BrowserBatchHardTimeoutError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "BrowserBatchHardTimeoutError";
  }
}
      ...
      if (timeoutMs !== undefined) {
        timer = setTimeout(
          () => {
            ...
            reject(
              new Error("Semaphore acquisition exceeded its hard deadline"),
            );
          },
...
export function withBrowserBatchHardTimeout<T>(
  ...
  return new Promise<T>((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      timedOut = true;
      reject(new BrowserBatchHardTimeoutError(message));
    }, timeoutMs);
    operation.then(
      (value) => {
        if (timedOut) {
...
export function parseBrowserBatchFetchInput(
  value: unknown,
): BrowserBatchFetchInput {
  ...
  if (body.requests.length > BROWSER_BATCH_FETCH_MAX_REQUESTS) {
    throw new Error(
      `requests must contain at most ${BROWSER_BATCH_FETCH_MAX_REQUESTS} items`,
    );
  }

  let totalBodyBytes = 0;
  const requests = body.requests.map(
    (requestValue, index): BrowserBatchFetchRequest => {
      if (
        !requestValue ||
      ...
      if (url.origin !== bootstrap.origin) {
        throw new Error(
          `requests[${index}].url must have the bootstrap origin`,
        );
      }
      ...
      if (bodyBytes > BROWSER_BATCH_FETCH_MAX_BODY_BYTES) {
        throw new Error(
          `requests[${index}].body exceeds the per-request byte limit`,
        );
      }
      ...
      for (const [name, headerValue] of Object.entries(
        ...
        if (!ALLOWED_HEADERS.has(normalizedName)) {
          throw new Error(
            `requests[${index}].headers contains disallowed header ${name}`,
          );
        }
        if (typeof headerValue !== "string" || headerValue.length > 512) {
          throw new Error(
            `requests[${index}].headers.${name} must be a short string`,
          );
        }
      ...
      if (!contentType || !ALLOWED_CONTENT_TYPES.has(contentType)) {
        throw new Error(
          `requests[${index}].headers.content-type is not allowed`,
        );
      }
  ...
  return {
    ...
    waitAfterLoadMs: boundedInteger(
      body.waitAfterLoadMs,
      0,
      0,
      30_000,
      "waitAfterLoadMs",
